...
> **Two models:** The **battery model** runs on its internal battery or on USB-C power. The **USB-C-only model** is identical but has no battery, so it always stays plugged in. This guide applies to both; wherever it says "connect USB-C power", the battery model can also run unplugged.

![Box contents](images/box-contents.jpg)
> *Image coming soon*

- 1x PlugNSat device (LilyGO T-Display S3, pre-flashed; battery model in the boxed version, USB-C-only model also available)
- 1x 3D-printed enclosure with 4 built-in magnets and 2 screw holes for wall mounting
- 1x desk stand (place it as-is, screw it, or stick it with double-sided tape)
